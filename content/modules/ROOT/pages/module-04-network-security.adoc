...
elif curl -fsSL --retry 3 --retry-delay 2 -o /tmp/test-dns.sh https://raw.githubusercontent.com/rhpds/deliver-openshift-ops-day/main/support/04-network-security/test-dns.sh; then
  bash /tmp/test-dns.sh
else
